...
module.exports.authenticateAccessTokenCookie = function authenticateAccessTokenCookie(req,res,next){
  ...
  var accessToken = req.cookies[req.app.get('stormpathAccessTokenCookieName')];
 ...
 * for other middleware to u se
 * @param  {Object} authenticationResult From the Node SDK
 * @param  {Object} req                  Express HTTP request
 * @param  {Object} res                  Express HTTP response
 */
module.exports.loginResponder = function loginResponder(authenticationResult,account,req,res){

  var accessTokenCookieName = req.app.get('stormpathAccessTokenCookieName');
  var accessTokenTtl = req.app.get('stormpathOauthTTL');
  var forceHttps = req.app.get('stormpathEnableHttps');
  var redirectUrl = req.app.get('stormpathRedirectUrl');
  var posLoginHandler = req.app.get('stormpathPostLoginHandler');

  var jwt = authenticationResult.getJwt().setTtl(accessTokenTtl);

  req.access_token = jwt;

  // Provide the user to the templating layer
  res.locals.user = account;
  // Provide the user on the requestrequest,
  // for other middleware to use
  req.user = account;

  // Create the cookie, but make sure we don't overwite any cookies that have
  // been set by anyone else

  var existing = (res.getHeader('Set-Cookie') || res.getHeader('set-cookie') || '') ;

  res.setHeader('set-cookie',
    [existing,
    accessTokenCookieName+'='+jwt.compact()+
    '; Expires='+new Date(jwt.body.exp*1000).toUTCString()+'; '+
    ((forceHttps || (req.protocol==='https'))?'Secure; ':'')+'HttpOnly;Path=/;']
  );

  if (posLoginHandler) {
    posLoginHandler(req.user, req, res, function() {
...
module.exports.getUser = function(req, res, next) {
  ...
  if(req.cookies && req.cookies[req.app.get('stormpathAccessTokenCookieName')]){
