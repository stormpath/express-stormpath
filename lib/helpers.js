  ...
  async.parallel([
    function(cb) {
      ...
      account.getApiKeys(function(err, apiKeys) {
        if (err) return accountCallback(new Error("Couldn't retrieve this account's apiKeys."));
    ...
    function(cb) {
      ...
      account.getCustomData(function(err, customData) {
        if (err) return accountCallback(new Error("Couldn't retrieve this account's customData."));
    ...
    function(cb) {
      ...
      account.getDirectory(function(err, directory) {
        if (err) return accountCallback(new Error("Couldn't retrieve this account's directory."));
    ...
    function(cb) {
      ...
      account.getGroups(function(err, groups) {
        if (err) return accountCallback(new Error("Couldn't retrieve this account's groups."));
    ...
    function(cb) {
      ...
      account.getGroupMemberships(function(err, groupMemberships) {
        if (err) return accountCallback(new Error("Couldn't retrieve this account's groupMemberships."));
    ...
    function(cb) {
      ...
      account.getProviderData(function(err, providerData) {
        if (err) return accountCallback(new Error("Couldn't retrieve this account's providerData."));
    ...
    function(cb) {
      ...
      account.getTenant(function(err, tenant) {
        if (err) return accountCallback(new Error("Couldn't retrieve this account's tenant."));
...
module.exports.getUser = function(req, res, next) {
  ...
  if (req.session && req.session.user) {
    req.app.get('stormpathClient').getAccount(req.session.user.href, function(err, account) {
      if (err) {
        req.session.destroy();
        return next();
      }
