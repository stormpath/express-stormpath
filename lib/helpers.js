...
module.exports.authenticateAccessTokenCookie = function authenticateAccessTokenCookie(req,res,next){
  var accessToken = new Cookies(req,res).get(req.app.get('stormpathConfig').web.accessTokenCookie.name);
...
module.exports.createToken = function(authenticationResult,account,req,res,next){
  ...
  var accessTokenCookieName = req.app.get('stormpathConfig').web.accessTokenCookie.name;
  ...
  var sessionExpirationTime = new Date(new Date().getTime() + (tti*1000));
...
module.exports.createPasswordSession = function(application,username,password,req,res,next){
    ...
    if(err){
      ...
      authenticationResult.getAccount(function(err,account){
        if(err){
          ...
          module.exports.createToken(authenticationResult,account,req,res,next);
        }
      });
    }
  });
...
module.exports.getUser = function(req, res, next) {
  ...
  var cookies = new Cookies(req,res);
  var accessToken = cookies.get(config.web.accessTokenCookie.name);
