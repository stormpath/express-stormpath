...
module.exports.getUser = function(req, res, next) {
  ...
  if (req.session && req.session.user) {
    ...
    // TODO: Remove this when we cut a 1.0.0 release.
    // If it's not a string, reset the cookies as they're probably using an
    // older version of our library.
    if (typeof req.session.user !== 'string') {
      req.app.get('stormpathLogger').info('Resetting user\'s cookies as they were using outdated session information from an older release.');
      req.session.destroy();
      return next();
    }
