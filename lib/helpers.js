...
module.exports.authenticateAccessTokenCookie = function authenticateAccessTokenCookie(req,res,next){
  var accessToken = new Cookies(req,res).get(req.app.get('stormpathAccessTokenCookieName'));
  var client = req.app.get('stormpathClient');
  var logger = req.app.get('stormpathLogger');

  req.app.get('jwsClaimsParser').parseClaimsJws(accessToken,function(err,jwt){
    if(err){
      logger.info('Failed to parse access token.');
      next();
    }else{
      req.accessToken = jwt;
      client.getAccount(jwt.body.sub,function(err,account){
        if(err){
 ...
 */
module.exports.createToken = function createToken(authenticationResult,account,req,res,next){
  var accessTokenCookieName = req.app.get('stormpathAccessTokenCookieName');
  var forceHttps = req.app.get('stormpathEnableHttps');
  var domain = req.app.get('stormpathSessionDomain');
  var httpOnly = req.app.get('stormpathSessionHTTPOnly');
  var path = req.app.get('stormpathSessionPath');
  var ttl = req.app.get('stormpathSessionTTL');
  var tti = req.app.get('stormpathSessionTTI');
  var cookies = new Cookies(req, res);
  var jwt = authenticationResult.getJwt().setTtl(ttl);
  var isSecure = forceHttps !== null ? forceHttps : (req.protocol === 'https');
  var sessionExpirationTime = new Date(new Date().getTime() + (tti*1000)).toUTCString();

  req.access_token = jwt;
  res.locals.user = account;
  req.user = account;

  cookies.set(accessTokenCookieName, jwt.compact(), {
    secure: isSecure,
    domain: domain,
...
module.exports.loginResponder = function(authenticationResult,account,req,res){
  var postLoginHandler = req.app.get('stormpathPostLoginHandler');
  var redirectUrl = req.app.get('stormpathLoginRedirect,Uri');
  module.exports.createToken(authenticationResult,account,req,res,function(){
    if (postLoginHandler) {
...
module.exports.registrationResponder = function(authenticationResult,account,req,res){
  var postRegistrationHandler = req.app.get('stormpathPostRegistrationHandler');
  var redirectUrl = req.app.get('stormpathRegisterRedirectUri');
  module.exports.createToken(authenticationResult,account,req,res,function(){
...
module.exports.getUser = function(req, res, next) {
  ...
  var cookies = new Cookies(req,res);
  var accessToken = cookies.get(req.app.get('stormpathAccessTokenCookieName'));

  if(accessToken){
    module.exports.authenticateAccessTokenCookie(req,res,next);
  }
