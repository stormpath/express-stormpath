...
module.exports.resendAccountVerificationEmail = function(req, res) {
  ...
  forms.resendAccountVerificationEmailForm.handle(req, {
    ...
    // verification email.
    success: function(form) {
      req.app.get('stormpathApplication').resendVerificationEmail({ login: form.data.email }, function(err) {
        if (err) {
          ...
          // helpers.render(view, res, { email: form.data.email });
        }
      });
    },
...
module.exports.forgotChange = function(req, res) {
  ...
  application.verifyPasswordResetToken(req.query.sptoken, function(err, result) {
    if (err) {
...
module.exports.verifyEmail = function(req, res) {
  ...
  client.getCurrentTenant(function(err, tenant) {
    if (err) {
      helpers.render(view, res, {err: 'During account verification, we attempted to fetch the current tenant, but this operation failed.'});

    } else {
      tenant.verifyAccountEmail(req.query.sptoken, function(err, account) {
        if (err) {
          var msg = 'A user attempted to verify their account email address using a token, but this token verification failed.';
          helpers.render(view, res, {error: msg});
          logger.info(msg);
        } else {
          req.app.get('stormpathClient').getAccount(account.href, function(err, acc) {
            if (err) {
              var msg = 'After a user verified their account, we were unable to retrieve their account details.';
              helpers.render(view, res, {error: msg});
              logger.info(msg);
            } else {
              res.locals.user = acc;
