...
module.exports.register = function(req, res) {
  ...
  if (req.method === 'POST' && accepts === 'json') {
    req.app.get('stormpathApplication').createAccount(req.body, function(err) {
      if (err) {
      ...
      if (err) {
        ...
        authenticationResult.getAccount(function(err, account) {
          if (err) {
            ...
            // Provide the user to the session middleware
            req.stormpathSession.user = account.href;
            // Provide the user to the templating layer
            res.locals.user = account;
            // Provide the user and access token on the request,
