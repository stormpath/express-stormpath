...
function createAccount(req, res, form) {
  var view = req.app.get('stormpathConfig').web.register.view;
...
module.exports.register = function(req, res) {
  ...
  if (req.method === 'POST' && accepts === 'json') {
    req.app.get('stormpathApplication').createAccount(req.body, function(err, account) {
      if (err) {
          ...
          if (err) {
            ...
            application.authenticateAccount({
              ...
              if (err){
                ...
                helpers.registrationResponder(authenticationResult,req.user,req,res);
              }
            });
          }
        });
...
module.exports.login = function(req, res) {
  if (req.user && req.app.get('stormpathConfig').web.login.enabled) {
    var url = req.query.next || req.app.get('stormpathConfig').web.login.nextUri;
  ...
  forms.forgotPasswordForm.handle(req, {
    ...
    success: function(form) {
      req.app.get('stormpathApplication').sendPasswordResetEmail(form.data.email, function(err) {
        ...
        res.redirect(req.app.get('stormpathConfig').web.forgotPassword.nextUri);
      });
    },
          ...
          if (form.data.password !== form.data.passwordAgain) {
            ...
            result.save(function(err, done) {
              if (err) {
                ...
                res.redirect(req.app.get('stormpathConfig').web.changePassword.nextUri);
              }
            });
          }
        },
