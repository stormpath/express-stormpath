  ...
  helpers.getFormViewModel('register', config, function (err, viewModel) {
    ...
    helpers.handleAcceptRequest(req, res, {
      ...
      'text/html': function () {
        ...
        return helpers.handleMultiTenantRequests(req, res, formActionUri, function () {
          switch (req.method) {
            // We should render the registration template.
            case 'GET':

              if (helpers.requiresOrganizationResolution(config, req)) {
                return helpers.parentDomainRedirect(req, res, config);
              }
