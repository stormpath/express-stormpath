...
module.exports = function (req, res) {
  ...
  assertionAuthenticator.authenticate(stormpathToken, function (err) {
    ...
    var signingKey = config.client.apiKey.secret;
    var parsedToken = nJwt.verify(stormpathToken, signingKey);
    ...
    function authenticateToken(callback) {
      ...
      parsedToken.body.status = 'AUTHENTICATED';

      stormpathTokenAuthenticator.authenticate({ stormpath_token: nJwt.create(parsedToken.body, signingKey).compact() }, function (err, authenticationResult) {
        if (err) {
    ...
    switch (tokenStatus) {
      ...
      case 'LOGOUT':
        ...
        var nextUri = config.web.logout.nextUri;
        var nextQueryPath = url.parse(params.next || '').path;
        res.redirect(302, nextQueryPath || nextUri);
        break;
