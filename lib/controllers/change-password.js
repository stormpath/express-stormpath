        ...
        forms.changePasswordForm.handle(req, {
          ...
          success: function (form) {
            if (form.data.password !== form.data.passwordAgain) {
              return helpers.render(req, res, view, { error: 'Passwords do not match.', form: form, urls: urls });
            }
            ...
            result.save(function (err) {
              if (err) {
                logger.info('A user attempted to reset their password, but the password change itself failed.');
                return helpers.render(req, res, view, { error: err.userMessage, form: form, urls: urls });
              }

              if (!helpers.isApp(req) && !helpers.allowBrowser(req, res))
                return helpers.render(req, res, view, {status: "success-in-browser", form: form, urls: urls });
          ...
          error: function (form) {
            ...
            if (form.data && !form.data.password && !form.data.passwordAgain) {
              return helpers.render(req, res, view, { form: form, action: action, urls: urls });
            }

            var formErrors = helpers.collectFormErrors(form);
            helpers.render(req, res, view, { form: form, formErrors: formErrors, urls: urls });
          },
          // If we get here, it means the user is doing a simple GET request, so we
          // should just render the forgot password template.
          empty: function (form) {
            helpers.render(req, res, view, { form: form, urls: urls });
          }
        });
      });
    }
