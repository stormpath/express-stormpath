            ...
            result.save(function (err) {
              ...
              if (config.web.changePassword.autoLogin) {
                ...
                return authenticator.authenticate({
                  ...
                  if (err) {
                    logger.info('A user attempted to reset their password, but when trying to login authentication failed.');
                    return helpers.render(req, res, view, { error: err.userMessage, form: form });
                  }

                  authResult.getAccount(function (err, account) {
                    if (err) {
                      logger.info('A user attempted to reset their password, but if failed when trying to retrieve the authenticated account.');
                      return helpers.render(req, res, view, { error: err.userMessage, form: form });
                    }

                    helpers.expandAccount(account, config.expand, logger, function (err, expandedAccount) {
                      if (err) {
                        logger.info('A user attempted to reset their password, but it failed when trying to expand the authenticated account.');
                        return helpers.render(req, res, view, { error: err.userMessage, form: form });
                      }
