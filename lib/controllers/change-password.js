          ...
          forms.changePasswordForm.handle(req, {
            ...
            success: function (form) {
              ...
              if (form.data.password !== form.data.passwordAgain) {
                ...
                return helpers.render(req, res, view, _.extend(viewData, {error: 'Passwords do not match.', form: form, urls: urls, user: user, email_groups: email_groups}));
              }
              ...
              return resetPasswordWithRecoveryToken(client, result, form.data.password, function (err) {
                if (err) {
                  ...
                  return helpers.render(req, res, view, _.extend(viewData, {error: err.userMessage, form: form, urls: urls, user: user, email_groups: email_groups}));
                }
            ...
            error: function (form) {
              ...
              helpers.render(req, res, view, _.extend({form: form, formErrors: formErrors, action: action, urls: urls, user: user, email_groups: email_groups}));
            },
            // If we get here, it means the user is doing a simple GET request, so we
            // should just render the forgot password template.
            empty: function (form) {
              helpers.render(req, res, view, _.extend({form: form, organization: req.organization, urls: urls, user: user, email_groups: email_groups}));
            }
          });
        });
      });
