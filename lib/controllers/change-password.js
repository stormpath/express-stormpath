          ...
          form.handle(req, {
            ...
            success: function (form) {
              ...
              if (form.data.password !== form.data.passwordAgain) {
                ...
                return helpers.render(req, res, view, _.extend(viewData, {urls: urls, user: user, action: action, email_groups: email_groups}));
              }
              ...
              return resetPasswordWithRecoveryToken(client, result, form.data.password, function (err) {
                if (err) {
                  ...
                  return helpers.render(req, res, view, _.extend(viewData, {urls: urls, user: user, email_groups: email_groups}));
                }

                if (action == 'invite')
                  config.web.changePassword.signupForEmail(user.email, req.body.email_interests);
                ...
                if (config.web.changePassword.autoLogin) {
                  var options = {
                    username: result.email || req.query.email,
            ...
            error: function (form) {
              ...
              helpers.render(req, res, view, {form: form, formErrors: formErrors, action: action, urls: urls, user: user, email_groups: email_groups});
            },
            // If we get here, it means the user is doing a simple GET request, so we
            // should just render the forgot password template.
            empty: function (form) {
              helpers.render(req, res, view, {form: form, organization: req.organization, urls: urls, user: user, email_groups: email_groups});
            }
          });
        });
      });
