...
/**
 * Find the href of an Organization, by nameKey
 *
 * @param  {object}   client - Stormpath client instance.
 * @param  {string}   nameKey - The name key of the organization.
 * @param  {Function} callback - Function to call with (err, [accountStoreHref])
 */
function resolveOrganizationHref(client, nameKey, callback) {
  client.getOrganizations({nameKey: nameKey}, function (err, collection) {
    if (collection && collection.items.length === 1) {
      callback(null, collection.items[0].href);
    } else {
      callback({ userMessage: 'Invalid Organization' });
    }
  });
}

/**
 * Given a login form post, translates the property names to ones that can be
 * used for an authentication attempt against the applications /oauth/token
 * endpoint
 *
 * @method
 * @private
 *
 * @param {object} client - Stormpath client instance.
 * @param {string} application - Stormpath application to authenticate against.
 * @param {object} credentials - The posted form body.
 * @param {function} callback - Function to call when transformation is ready.
 */
function transformAuthenticationAttempt(client, application, credentials, callback) {
  ...
  if (!credentials.username || !credentials.password) {
    return process.nextTick(callback.bind(null, new Error('Invalid username or password.')));
  }

  if (credentials.organizationNameKey) {
    return resolveOrganizationHref(client, credentials.organizationNameKey, function (err, accountStoreHref) {
      credentials.accountStore = accountStoreHref;
      callback(err, credentials);
    });
  }

  return process.nextTick(callback.bind(null, null, credentials));
}
 ...
 */
function authenticate(client, application, credentials, callback) {

  transformAuthenticationAttempt(client, application, credentials, function (err, credentials) {
    if (err) {
      return callback(err);
    }
    ...
    'application/json': function () {
      switch (req.method) {
        ...
        case 'POST':
          ...
          authenticate(client, application, req.body, function (err, result) {
            if (err) {
              return helpers.writeJsonError(res, err);
            }
    ...
    'text/html': function () {
      ...
      forms.loginForm.handle(req, {
        ...
        success: function (form) {
          authenticate(client, application, form.data, function (err, result) {
            if (err) {
