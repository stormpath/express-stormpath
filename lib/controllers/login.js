...
module.exports = function (req, res, next) {
  ...
  helpers.getFormViewModel('login', config, accountStoreParent, function (err, viewModel) {
    if (err) {
          ...
          forms.loginForm.handle(req, {
            ...
            success: function (form) {
              ...
              helpers.authenticate(form.data, req, res, function (err) {
                if (err) {
                  ...
                  return helpers.renderLoginForm(req, res, { form: form, error: err.userMessage || err.message });
                }
            ...
            error: function (form) {
              ...
              if (form.data && !form.data.login && !form.data.password) {
                return helpers.renderLoginForm(req, res, {form:form});
              }

              helpers.renderLoginForm(req, res, { form: form, formErrors: helpers.collectFormErrors(form) });
            },
            // If we get here, it means the user is doing a simple GET request, so we
            // should just render the login template.
            empty: function (form) {
              helpers.renderLoginForm(req, res, {form:form});
            }
          });
        });
      }
