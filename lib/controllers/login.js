...
/**
 * Set the account store in the account data.
 *
 * @function
 *
 * @param {Object} accountData - Account data to set account store in.
 * @param {Object} accountStore - Account store to retrieve href from.
 */
function setAccountStoreByHref(accountData, accountStore) {
  return accountData.accountStore = accountStore.href;
}

function setMultiTenancyFields(req, viewModel) {
  var organizationNameKeyField = null;
  var config = req.app.get('stormpathConfig');

  viewModel.form.fields.forEach(function (field) {
    if (field.name === 'organizationNameKeyField') {
      organizationNameKeyField = field;
    }
  });

  if (config.web.multiTenancy.enabled && organizationNameKeyField && req.organization) {
    organizationNameKeyField.enabled = false;
  }
}
...
module.exports = function (req, res, next) {
  ...
  helpers.getFormViewModel('login', config, function (err, viewModel) {
    ...
    setMultiTenancyFields(req, viewModel);
