        ...
        forms.forgotPasswordForm.handle(req, {
          ...
          success: function (form) {
            var opts = {
              ...
              method: 'GET',
              url: okta_url + '/api/v1/users/' + email
            };
            request.get(opts, function(err, user_inquiry_resp) {

              var resp = JSON.parse(user_inquiry_resp && user_inquiry_resp.body);
              var err_code = resp && resp.errorCode;
              if (err_code && err_code == 'E0000007') {
                ...
                request.post(opts, function(err, recover_password_res) {
                  if (!err && recover_password_res.statusCode != 200)
                    err = new Error('Okta pwd recovery HTTP error ' + recover_password_res.statusCode + ': ' + (recover_password_res.body ? (recover_password_res.body.message || recover_password_res.body) : ''));
...
function sendMissingEmail(email, callback) {
  var subject = 'Daycast Password Reset Attempt';
  var body = 'We\'ve received a request to reset the password for this email address. ';
  body += 'But, we do not have a username with this email address on file. ';
  body += 'Please try again with another email. ';
