...
function sendForgotEmail(email, token, callback) {
  ...
  getTokenTimeout(function(token_lifetime) {
    sendEmail(email, 'Reset Your Daycast Password', reset_link, token_lifetime, callback);
  });
}
...
function getTokenTimeout(callback) {
  okta('GET', okta_url + '/api/v1/policies?type=PASSWORD', null, function(err, resp, body) {
    ...
    var stormpath_policy = resp_body.find(function(policy){return policy.name == 'Stormpath Administrators-Policy'});
...
function sendEmail(to_email, subject, reset_link, token_lifetime, callback) {
  var api_key = process.env.MANDRILL_API_KEY;
  var base_url = process.env.MANDRILL_API_URL;

  /* fallback, this will show what node can see */
  var os_name = os.type();
  var os_pretty_names = {'Darwin': 'macOS', 'Windows_NT': 'Windows'};
  var os_pretty_name = os_pretty_names[os_name];
  var os_version = os.release();

  /* We can customize the response from macOS and Windows because they are known quantities */
  try {
    if (os_name == 'Darwin') {
      os_version = child_process.execSync('sw_vers -productVersion', {encoding: 'utf-8'});
      os_pretty_name = child_process.execSync('sw_vers -productName', {encoding: 'utf-8'});
    }
    else if (os_name == 'Windows_NT') {
      os_version = child_process.execSync('ver', {encoding: 'utf-8'});
      os_pretty_name = '';
    }
  }
  catch (e) {
    logger.info('getting real OS info failed, using fallback');
  }
  ...
  opts.body = {
    ...
    message: {
      ...
      global_merge_vars: [
        ...
        {
          name: 'reset_window',
          content: (token_lifetime / 60).toString()
        }
      ]
    }
  };
