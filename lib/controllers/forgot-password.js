...
function sendForgotEmail(email, token, callback) {
  ...
  getTokenTimeout(function(token_lifetime) {
    getIPAddress(function(ip_address) {
      sendEmail(email, 'Reset Your Daycast Password', reset_link, token_lifetime, ip_address, callback);
    });
  });
}

function getIPAddress(callback) {
  request.get('https://api.ipify.org', function(err, resp, body) {
    callback(body);
  })
}
...
function sendEmail(to_email, subject, reset_link, token_lifetime, ip_address, callback) {
