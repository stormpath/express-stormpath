...
module.exports.init = function(app, opts) {
  ...
  client.on('ready',function() {
    ...
    if (config.web.changePassword.enabled) {
      router.get(config.web.changePassword.uri, controllers.resetPassword);
      router.post(config.web.changePassword.uri, bodyParser.json({ limit: '200kb' }), bodyParser.urlencoded({ extended: false }), controllers.resetPassword);
    }

    if (config.web.verifyEmail.enabled) {
      router.get(config.web.verifyEmail.uri, controllers.verifyEmail);
      router.post(config.web.verifyEmail.uri, controllers.verifyEmail);
    }
