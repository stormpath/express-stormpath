...
module.exports.init = function (app, opts) {
  ...
  client.on('ready', function () {
    ...
    if (web.multiTenancy.enabled) {
      var organizationResolver = null;

      if (web.register.form.fields.organizationNameKey.enabled !== false) {
        web.register.form.fields.organizationNameKey.enabled = true;
      }

      if (web.login.form.fields.organizationNameKey.enabled !== false) {
        web.login.form.fields.organizationNameKey.enabled = true;
      }

      if (web.multiTenancy.organizationResolver) {
        organizationResolver = web.multiTenancy.organizationResolver;
      } else {
        // We need to use the body parser in order to be able to
        // access the post body for the register and login routes.
        if (web.register.enabled) {
          router.post(web.register.uri, bodyParser.formOrJson());
        }

        if (web.login.enabled) {
          router.post(web.login.uri, bodyParser.formOrJson());
        }

        organizationResolver = middleware.defaultOrganizationResolver;
      }
