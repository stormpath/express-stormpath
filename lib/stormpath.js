...
module.exports.init = function (app, opts) {
  ...
  client.on('ready', function () {
    ...
    if (web.logout.enabled) {
      if (web.saml.enabled) {
        addGetRoute(web.logout.uri, controllers.initiateSaml({logout: true}));
      }
    ...
    if (web.saml.enabled) {
      addGetRoute(web.saml.uri, controllers.initiateSaml());
      addGetRoute(web.saml.verifyUri, controllers.verifySaml);
    }
