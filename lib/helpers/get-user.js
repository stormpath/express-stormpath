...
module.exports = function (req, res, next) {
  ...
  if (cookies && cookies[config.web.accessTokenCookie.name]) {
    ...
    authenticator.authenticate(cookies[config.web.accessTokenCookie.name], function (err, authenticationResult) {
      if (err) {
        logger.info('Failed to authenticate the request. Invalid access_token found.');

        if (!cookies[config.web.refreshTokenCookie.name]) {
          return next();
        }

        return new stormpath.OAuthRefreshTokenGrantRequestAuthenticator(application).authenticate({ refresh_token: cookies[config.web.refreshTokenCookie.name] }, function (err, refreshGrantResponse) {
          if (err) {
  ...
  } else if (cookies && cookies[config.web.refreshTokenCookie.name]) {
    new stormpath.OAuthRefreshTokenGrantRequestAuthenticator(application).authenticate({ refresh_token: cookies[config.web.refreshTokenCookie.name] }, function (err, refreshGrantResponse) {
      if (err) {
        logger.info('Failed to refresh an access_token given a refresh_token.');
        return next();
      }
