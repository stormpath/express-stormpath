...
var getAppUserById = require('../okta/get-app-user-by-id');
...
module.exports = function authenticate(options, req, res, callback) {
  ...
  function continueWithAuthentication() {
    ...
    passwordGrant(config, options, function (err, oauthAccessTokenResult) {
      ...
      var accessTokenAuthenticator = new AccessTokenAuthenticator().forIssuer(issuer).withLocalValidation();

      accessTokenAuthenticator.authenticate(oauthAccessTokenResult.access_token, function (err, jwt) {
        ...
        getAppUserById(config.org, config.application.id, config.apiToken, jwt.body.uid, (err, user) => {
          if (err) {
            logger.info(err);
            return callback(err);
          }
