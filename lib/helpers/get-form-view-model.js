 ...
 */
function getAccountStores(accountStoreParent, callback) {
  var options = { expand: 'accountStore' };

  // Get account store mappings.
  accountStoreParent.getAccountStoreMappings(options, function (err, accountStoreMappings) {
    if (err) {
      return callback(err);
    }
 ...
 * @param {Function} callback - Callback function (error, providers).
 */
function getProviders(config, accountStoreParent, callback) {

  getAccountStores(accountStoreParent, function (err, accountStores) {
    if (err) {
      return callback(err);
    }
 ...
 */
function getFormViewModel(formName, config, accountStoreParent, callback) {
  var cacheKey = formName + '_' + accountStoreParent.href;
  ...
  var cachedModel = cache.getCachedItem(cacheKey, cacheTtl);
  ...
  getProviders(config, accountStoreParent, function (err, providers) {
    if (err) {
      return callback(err);
    }
    ...
    cache.setCachedItem(cacheKey, model);
